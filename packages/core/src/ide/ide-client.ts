...
const SERVER_FILE_PREFIX = 'gemini-ide-server-';
...
export class IdeClient {
  ...
  > {
    ...
    const serverFiles = files.filter((f) => f.startsWith(SERVER_FILE_PREFIX));
    ...
    if (serverFiles.length === 1) {
      const expectedFile = `${SERVER_FILE_PREFIX}${this.ideProcessInfo.pid}.json`;
    ...
    // Rule 2: find all files that match "gemini-ide-server-${this.ideProcessInfo.pid}".
    ...
    const expectedPrefix = `${SERVER_FILE_PREFIX}${this.ideProcessInfo.pid}`;
    const otherServerFiles = serverFiles.filter(
      (file) => !file.includes(expectedPrefix)
    );
    let filteredConfigs = [];
